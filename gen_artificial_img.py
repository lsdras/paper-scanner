...
class IMG_COMBIN:
    ...
    def resize_foreground(self):
        rows,cols = self.front_img.shape
    ...
    def make_random_4points(self,height,width):
        ...
        y1 = np.random.randint(0.125*width,0.2*width)/row_ratio
        x1 = np.random.randint(0.125*width,0.2*height)/col_ratio
        y2 = np.random.randint(0.8*width,0.875*width)/row_ratio
        x2 = np.random.randint(0.125*height,0.2*height)/col_ratio
        y3 = np.random.randint(0.8*width,0.875*width)/row_ratio
        x3 = np.random.randint(0.8*height,0.875*height)/col_ratio
        y4 = np.random.randint(0.125*width,0.2*width)/row_ratio
        x4 = np.random.randint(0.8*height,0.875*height)/col_ratio
...
if __name__ == '__main__':
    ...
    combin, mask = Img_combin.combin_back_front()
    cv2.imwrite("combine.png",combin)
